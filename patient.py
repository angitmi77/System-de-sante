...
class EspacePatient:
    ...
    def voir_mes_rendez_vous(self):
        ...
        self.cursor.execute('''
            SELECT rv.date_rdv, rv.heure_rdv, m.nom_complet, m.specialite, rv.urgent, rv.statut
            FROM rendez_vous rv
        ...
        if not rendez_vous:
            ...
            tree = ttk.Treeview(fenetre_rdv, columns=columns, show="headings", height=12)
            ...
            for rdv in rendez_vous:
                urgent_text = "OUI" if rdv[4] else "NON"
                tree.insert("", tk.END, values=(rdv[0], rdv[1], f"Dr. {rdv[2]}", rdv[3], urgent_text, rdv[5]))
